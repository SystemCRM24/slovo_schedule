...
async def get_schedules(date_range: DateRange = Depends()):
    ...
    try:
        start = (
            date_range.start.astimezone(Settings.TIMEZONE)
            if date_range.start.tzinfo
            else date_range.start.replace(tzinfo=Settings.TIMEZONE)
        )
        end = (
            date_range.end.astimezone(Settings.TIMEZONE)
            if date_range.end.tzinfo
            else date_range.end.replace(tzinfo=Settings.TIMEZONE)
        )
        ...
        for appointment in response:
            try:
                ...
                if not all(
                    ...
                    logging.debug(
                        f"Пропуск записи с id={appointment.get('id')} из-за неполных данных"
                    )
                    continue

                specialist_id = appointment["assignedById"]
                start_time = datetime.fromisoformat(
                    appointment[constants.uf.appointment.start]
                )
                start_of_day = start_time.replace(
                ...
                logging.debug(
                    f"Пропуск записи с id={appointment.get('id')} из-за ошибки: {str(e)}"
                )
                continue

        schedule_list = [
            ScheduleResponse(
                specialist_id=specialist_id, date=date, appointments=appointments
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка при получении расписания записей специалистов: {str(e)}",
        )


@router.get(
    "/get_work_schedules", status_code=200, response_model=List[WorkScheduleResponse]
)
        ...
        for schedule in response:
            try:
                ...
                if not all(
                    ...
                    logging.debug(
                        f"Пропуск расписания с id={schedule.get('id')} из-за неполных данных"
                    )
                    continue
                ...
                for interval in raw_intervals:
                    try:
                        ...
                        intervals.append(
                            WorkInterval(
                                start=datetime.fromtimestamp(float(start) / 1000)
                                .astimezone(Settings.TIMEZONE)
                                .isoformat(),
                                end=datetime.fromtimestamp(float(end) / 1000)
                                .astimezone(Settings.TIMEZONE)
                                .isoformat(),
                            )
                        )
                        ...
                        logging.debug(
                            f"Пропуск интервала в расписании с id={schedule.get('id')} из-за ошибки: {str(e)}"
                        )
                        continue
                ...
                if not intervals:
                    logging.debug(
                        f"Пропуск расписания с id={schedule.get('id')} из-за отсутствия валидных интервалов"
                    )
                    continue
                ...
                logging.debug(
                    f"Пропуск расписания с id={schedule.get('id')} из-за ошибки: {str(e)}"
                )
                continue

        work_schedule_list = [
            WorkScheduleResponse(
                specialist_id=specialist_id, date=date, schedule=schedule
            )
